...
export default async function AvmPostsRow() {
  try {
    const categories = await fetchCategories();
    const avmler = categories.find(category => category.slug === 'avmler');

    if (!avmler) return null;

    const posts = await fetchPosts({
      categoryId: avmler.id,
      per_page: 10,
      page: 1,
      orderby: 'date',
      order: 'desc',
    ...
    const parentCategory = avmler.parentId
      ? categories.find(category => category.id === avmler.parentId)
      : undefined;
    const categoryHref = parentCategory
      ? `/${parentCategory.slug}/${avmler.slug}`
      : `/${avmler.slug}`;
    const fallbackImage = (title: string) => getDummyImageForCategory(avmler.slug, title);

    return (
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold text-brand-soft-blue">AVM&apos;ler</h2>
          <Link
            href={categoryHref}
    ...
    console.error('Error in AvmPostsRow:', error);
    return null;
  }
}
