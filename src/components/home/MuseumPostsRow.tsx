...
export default async function MuseumPostsRow() {
  try {
    const categories = await fetchCategories();
    const museums = categories.find(category => category.id === 49);
    ...
    const parentCategory = museums.parentId
      ? categories.find(category => category.id === museums.parentId)
      : undefined;
    const categoryHref = parentCategory
      ? `/${parentCategory.slug}/${museums.slug}`
      : `/${museums.slug}`;

    return (
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold text-brand-soft-blue">Müzeler</h2>
          <Link
            href={categoryHref}
        ...
        <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2">
          {posts.slice(0, 10).map(post => {
            const fallback = getDummyImageForCategory(museums.slug, post.title);
    ...
    console.error('Error in MuseumPostsRow:', error);
    return null;
  }
}
