...
export default async function HospitalPostsRow() {
  try {
    const categories = await fetchCategories();
    const hospitals = categories.find(category => category.slug === 'hastaneler');

    if (!hospitals) return null;

    const posts = await fetchPosts({
      categoryId: hospitals.id,
      per_page: 10,
      page: 1,
      orderby: 'date',
      order: 'desc',
    ...
    const parentCategory = hospitals.parentId
      ? categories.find(category => category.id === hospitals.parentId)
      : undefined;
    const categoryHref = parentCategory
      ? `/${parentCategory.slug}/${hospitals.slug}`
      : `/${hospitals.slug}`;

    return (
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold text-brand-soft-blue">Hastaneler</h2>
          <Link
            href={categoryHref}
        ...
        <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2">
          {posts.slice(0, 10).map(post => {
            const fallback = getDummyImageForCategory(hospitals.slug, post.title);
    ...
    console.error('Error in HospitalPostsRow:', error);
    return null;
  }
}
